...
export function showCredits(stage: HTMLElement, deps: CreditsDeps): Disposer {
  ...
  const p = panel({ rank: 'inspect', title: 'Credits' });
  wrap.appendChild(p.el);

  // --- Made by --------------------------------------------------------------
  const made = section(p.body, 'Made by');
  ...
  // --- Built with -------------------------------------------------------------
  const built = section(p.body, 'Built with');
  ...
  for (const font of CREDITS.fonts) {
    ...
    summary.addEventListener('click', () => {
      ...
      deps
        ...
        .catch(() => {
          licenceBody.textContent = 'licence text unavailable offline';
        });
    });
    type.appendChild(d);
  }
  ...
  // --- Art and models -----------------------------------------------------
  const art = section(p.body, 'Art and models');
  ...
  // --- Licence --------------------------------------------------------------
  const licence = section(p.body, 'Licence');
  const licenceP = document.createElement('p');
  licenceP.textContent = `Code: ${CREDITS.codeLicence}. Art and data: ${CREDITS.artLicence}.`;
  licence.appendChild(licenceP);

  const build = document.createElement('p');
  build.className = 'rl-dim rl-credits__build';
  build.textContent = `Build ${deps.build}`;
