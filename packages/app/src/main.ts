// @lions/app — M0 sandbox shell. Composes sim + render + data.
// Owns the real-time loop: the sim ticks at a fixed 20 Hz from an
// accumulator; the renderer interpolates between ticks (invariant 1).
...
async function main(): Promise<void> {
  ...
  const overlay = new DebugOverlay(document.body, sim, () => renderer.selection);
