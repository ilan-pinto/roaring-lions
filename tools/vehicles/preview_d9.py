"""Preview the D9 Dov on the locked rig, over ground, in palette colour.

    /Applications/Blender.app/Contents/MacOS/Blender --background \
        --python tools/vehicles/preview_d9.py

Writes art/showcase/d9_mock{,_nose,_side,_tail}.png. Does not save the .blend: the
ground plane, the mock camera and these materials are for looking at, not for
shipping, and render_vehicle.py builds its own camera and materials from scratch.

Why three facings and not one. The thing that can sink this design is the art
gate's pairwise silhouette IoU against TNK_HULL, EITAN_HULL and NAMER_HULL -- three
tracked olive boxes drawing at the same size. A single profile view flatters the
moldboard, because in profile it is unmistakable; the question is whether the blade,
the stack and the ripper still separate it at three-quarter facings, which is where
the outlines converge. So the model turns under a fixed camera and light, exactly as
it does in play.

Derived from preview_technical.py, which shares the rig setup but not the reasoning:
that truck is limestone on limestone and worries about vanishing into pale ground.
This one is olive and reads clearly; its problem is looking like a tank.
"""
import math
import os
import sys
...
OUT = os.path.abspath("art/showcase/d9_mock.png")
...
PAL = {
    ...
    "metal": "#8E9491",   # gunmetal.1 -- blade, stack, arms: bright so the blade reads
