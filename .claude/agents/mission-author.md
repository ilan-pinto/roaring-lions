...
## Briefings, and where the presentation questions live

Mission briefings are authored here, in mission JSON, but today they render as text
inside the HUD (`packages/app/src/ui/hud.ts`) — there is no briefing screen, no
debrief, and nothing between missions. Two open tickets own that and neither is yours:
**#94** owns the campaign's narrative, sequencing, and the fact that no briefing
references any prior mission's outcome; **#109** owns the presentation of it, and
explicitly defers to #94 for content.

So: keep writing briefings as tactical orders in the mission file. If a task asks for
narrative continuity between missions, or for briefing *presentation*, point at #94
and #109 rather than inventing a mechanism — and note that the ledger already carries
`roster.surviving_units`, `roe.mission_ratings` and `intel.marked_positions` but has
no narrative keys.
