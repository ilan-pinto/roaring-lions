...
## The economy questions in #109 are design, not tuning

#109 (C&C adoption) carries two open questions that look like tuning and are not:
whether the game adopts a **harvester loop** (GDD §3 says no — throughput, not
gathering) and whether intel abilities get **superweapon presentation**. The second
is presentation over a mechanic that already exists — satellite sweep 150, precision
strike 250 — so it costs you nothing. The first would change what the cost curve is
even pricing against.

If a task asks you to retune around either, note that #109 owns the decision and that
GDD §3 and §5.7 currently outrank the request. The economy anchors in §3 are
provisional against mission playtests; the *shape* of the economy is not.
