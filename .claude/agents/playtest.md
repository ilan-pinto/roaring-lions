...
## Feel and legibility findings have a home: #109

You drive the real UI, so you are the agent most likely to notice what #109 (C&C
adoption) is actually about. Two gaps it names are ones a playtest hits immediately:
**there is no minimap** anywhere in the app or renderer, and **there is no voice or
announcement layer** — `data/audio.json` holds eleven sets and every one is a weapon
or an impact, so nothing tells the player a unit was lost, a build finished, or a
flank is collapsing except the picture itself.

When a run stalls for a reason that is legibility rather than balance, say so and file
it against #109's lane 1 (or #52, which is the same gap with no stated target). Do not
recommend a tuning change for a problem that is really the player not being told
something.
