...
- `stepCombat`'s structure-engagement branch (`sim.ts:1991`) sets `engagedClose = true` unconditionally the instant a garrisoned building comes within a weapon's max range, where the unit-vs-unit branch just above it (`:1969`) correctly gates on `effectiveRangeSq`. Since `attackMove` halts on `engaging`, an attack-mover stalls to snipe any building at max range well short of where it was actually headed — measured cost ~3.7 minutes across two stalls in a Wadi Halam IV playtest before the mission's plan was re-sequenced around it. Needs a balance pass against the §5.7 targets before fixing, since it changes core combat behaviour for every attack-mover against every building on the map. Content authored before that fix should not be assumed to hit its `target_minutes` for a player who attack-moves through a garrisoned building rather than routing around it — Wadi Halam IV's measured 6.0 min came from a plan that deliberately avoids the stall.
...
- `stepObjectives`' `raze` branch (`mission.ts:1126-1130`) sets `complete` and never `failed`, so a `raze` primary whose target becomes unreachable leaves the mission unwinnable and unlosable at once — the same gap `evacuate_before` had before it grew a deadline. Wadi Halam V works around it rather than fixing it: it carries a `demo_squad` in `starting_force` as a fallback demolisher, unconditionally, because `spawnPlacement` never consults `unlock` — a squad gated behind a ledger key the player might not have would silently remove the only way to complete `raze_depot` on a fresh campaign. Needs the same failure-branch treatment as `evacuate_before` got.
