...
**Status at landing (2026-09-24, execution-plan session).** Approved under the lead's standing instruction to proceed on recommendation; the six open questions took the spec's own recommendations, and §10's numbers stay gated on the lead before Task 7. Entry condition (1) below is **met**: `fix/renderer-reseed` landed as PR #215. **Task 1 is already done**: PR #214 (`51924fd4`) gave `ui:shots` and `ui:routes` a `--port=<n>` flag and a `UI_SHOTS_PORT`/`UI_ROUTES_PORT` override, with a busy port refused (exit 2), so the executor skips Task 1 and uses those flags. The ports paragraph below also applies to the tools' own ports: nothing reuses a busy one now. One more entry condition was added after writing: **wait for the `fix/renderer-context-release` PR too.** It edits `bootBattlefield`'s boot and teardown in `main.ts`, where Task 2 extracts code, and it already makes `ThreeRenderer.dispose()` lose its context. Re-read that before writing the host's disposer, which may then need no explicit `loseContext()` of its own.
