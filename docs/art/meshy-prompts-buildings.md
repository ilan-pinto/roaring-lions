...
|---|---|---|
| `hall` (replaced `mosque` 2026-09-06; the mosque rows read 3071/198, 2939/186) | 3880/15 | directional, correct (258x) — the portico faces the camera |
...
| `apartment`, `camp`, `wall`, `house_wreck`, `clinic_wreck`, `hall_wreck` | — | **no `glass` role at all — unchecked**, named on the passing path |
...
**Civic hall status, 2026-09-06 (O10 closed).** `hall` replaces `mosque` on the same `m` tiles with the same numbers (900 hp/tile, garrison 3, `roe_penalty` 30 — the protected type, deliberately unchanged), renamed in place in `data/structures.json` so the determinism hash did not move. Export from the two supplied textured passes (the part-segmentation pass rejected as the clinic's was): 974k source verts to 12,264 standing (1.32 MB, 8.4 × 9.6 × 4.6 m at `REAL_METRES_HALL` 9.0, the 3×3 block's own width) and 20,992 wrecked (3.96 MB). The front was measured three ways before export and the facing gate reads 3880 px camera against 15 hidden (258x). The wreck is heavy and it is the source's doing, not a setting's: five decimation routes all floor at ~47k polys because the destroyed pass is thousands of small debris islands (numbers in `export_meshy_hall.py`); lighter means deleting islands below a face count, which is an art call about how much rubble stays. Retired with it: `mosque.glb`, `mosque_wreck.glb`, `art/src/buildings/mosque.blend`, `assets/sprites/BLD_MOSQUE/` and `tools/buildings/patch_mosque.py`; the Pixi / `&nomesh` path draws the generic box. The Wadi Halam zone is `hall_block` and the three briefings say "the civic hall".

**Clinic status, 2026-09-06.** Landed at `77390ab`: 961k source verts decimated to 11.9k standing / 9.1k wrecked, scale area-matched to the 4×4 block (idle 10.2 × 14.1 × 6.7 m), `roe_penalty` 6 — the costly tier, level with `house`, **not** protected: it shipped at 25 (above `PROTECTED_ROE`, drawing the mosque's protected mark) and the lead's call the same day was "clinic should not be protected", so the mark is gone and only the `clinic` zone Beit Sahwan III already flags charges fire there. Playtest verdicts and Beit Sahwan III's ROE (84) are unchanged: no scripted plan destroys the block. Two things the gates said that are worth knowing. `validate:meshes` passes with it (48 meshes, no silhouette collision). And **the visual gate cannot see it**: the `quiet` scenario frames `town_center`, but that block sits under fog in an order-free sandbox — CI read 0 px, bit-identical, on the commit that placed it. A local `pnpm golden-baseline` run reported ~10.9k px in `quiet` and attributed them to the clinic; those are the HUD strip against the macOS baseline last blessed 2026-09-03. Nothing was blessed.
