...
| 3 | Shai portrait `assets/ui/shai_portrait.png` (512×640) | **MISSING** | **no gate** — `tools/validate_assets.py` defaults to `--sprites assets/sprites`, so `assets/ui/` is ungated, same footing as `menu_host_plate.jpg` | generative, PR disclosure required (`CONTRIBUTING.md`); brief in `2026-08-21-commander-brief-design.md` |
...
| 10 | `assets/ui/menu_host_plate.jpg` (an engine-captured plate, photographed live from the menu's scene host — `pnpm plate:host`, scene-host plan task 7; supersedes the Phase 0 `menu_plate.jpg` / `pnpm plate:capture`) | **PRESENT** | — | — |
