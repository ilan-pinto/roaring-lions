...
| **civilians** | **`refuge` changes from `civ_refuge` to `civ_collection`** (this moves where *all* of IV's civilians flee, from `[22,45]` to `[29,33]` — three tiles from the player start, and better). Groups: the two shipped (`×2 [28.5,14.5]`, `×2 [24.5,33.5]`) **+ `hostages`**: `civilians ×4` at `[24.5,14.5]`, `group: "hostages"`. Bodies land on `(24,14) (25,14) (26,14) (24,15)`, all `.`, all clear of every other placement |
