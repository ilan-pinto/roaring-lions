...
|---|---|---|
| unit JSON ×3 | **PRESENT** (2026-09-13) | `data/units/kdf/{breach_team,scout_shachaf,apc_kipod}.json` · `pnpm validate:data` (after the schema edit), `tools/validate_balance.py` |
| `breach_team` mesh | **PRESENT** (2026-09-13) | `art/meshes/breach_team.glb` · builder in `tools/units/teams.py` `TEAMS`, figures in `tools/units/rig.py` `TEAM_FIGURES` **and** `SUPPORTED_TEAMS` (`_check_team_figures_against_teams` asserts they match), then `tools/export_mesh_team.py -- breach_team`. Gate `pnpm validate:meshes`. **Named risk:** silhouette IoU ≤ 0.88 against 46 meshes and 36 sprites, nearest neighbours `demo_squad.glb` and `yahalom_squad.glb`. The shield and pole charge must separate it, not the texture |
| `breach_team` sheet | **PRESENT** (2026-09-14) | `assets/sprites/INF_BREACH/` · `tools/render_team.py -- breach_team` · `pnpm validate:assets`. Portrait, `&nomesh`/Pixi billboard, `down`/`wreck` |
| `scout_shachaf` / `apc_kipod` sheets | **PRESENT** (2026-09-14) | `assets/sprites/SHACHAF_HULL/`, `assets/sprites/KIPOD_HULL/` · `tools/render_scout_shachaf.py`, `tools/render_apc_kipod.py` (hull only, no turret split; same `real_metres` as the GLB export) · `pnpm validate:assets` |
| `scout_shachaf` mesh | **PRESENT** (2026-09-13) | `art/meshes/vehicles/scout_shachaf.glb` · `tools/export_mesh_vehicle.py` (whose only shipped output is `apc_eitan.glb`) or a supplied Meshy `.blend` exported as the textured buildings are. Gate `pnpm validate:meshes` |
...
| `apc_kipod` mesh | **PRESENT** (2026-09-13) | `art/meshes/vehicles/apc_kipod.glb` · same pipeline. Reactive plate must read in a 64 px black silhouette or it is invisible doctrine |
| both vehicles' wreck | **MESH wreck PRESENT** (2026-09-15); billboard wreck PRESENT (2026-09-14) and now only `&nomesh`/Pixi | Every line of this row said the opposite until 2026-09-15 and all of it is superseded — mesh vehicles no longer declare zero animations, the clone is no longer pruned at `alive == 0`, `addWreck` no longer falls back to a billboard for them, and wreck geometry is no longer the roster-wide debt. `pnpm wreck:meshes` gives all eleven vehicle GLBs a `death_root` and the clips `idle`/`wreck`, and `units/mesh-vehicle-death.ts` fades, plays the clip and keeps a persistent charred `MeshWreck`. A destroyed Shachaf or Kipod now leaves a slumped, charred 3D hull on `three`, and the 2D `wreck` sprite each hull sheet carries is what `&nomesh` and `?renderer=pixi` still draw. See CLAUDE.md's "Known scaling debts" mesh-vehicle-death paragraph for the contract, the recipe and what is genuinely still open (real damaged geometry, D1's later per-vehicle art pass) |
| `SPRITE_MAP` ×3 | **PRESENT** (2026-09-14) | `packages/app/src/main.ts` `SPRITE_MAP`: `breach_team`, `scout_shachaf`, `apc_kipod`. Art existing is not art drawing — three complete sheets once shipped and drew nothing |
| mesh-catalogue ×3 | **PRESENT** (2026-09-13) | `packages/app/src/mesh-catalogue.ts` `RIGGED_UNIT_MESHES` (Tzinah) / `VEHICLE_UNIT_MESHES` (Shachaf, Kipod); pinned by `mesh-catalogue.test.ts` |
| unit names | **PRESENT** (2026-09-13, `recon` added to `kinds.vehicle_roles` with the units) | `data/campaign/names.json` `kinds.vehicle_roles` is `[apc, ifv, mbt]`, so `scout_shachaf` (role `recon`) would be named as a **squad**. Add `recon`. `apc_kipod` is covered; `breach_team` correctly falls through to `squads` |
| audio | MISSING, out of scope — portraits are PRESENT since the sheets (the chip frame, `ui/portrait.ts`) | No unit in the tree has a bark or a portrait (GH-110); the dock's portrait frame degrades to `null` by design |
