...
| both vehicles' wreck | **billboard wreck PRESENT** (2026-09-14), mesh wreck still absent | Mesh vehicles declare zero animations; `updateVehicleMeshes` prunes the clone at `alive == 0` and `addWreck` falls back to a billboard `UnitWreck` — which now exists, since each hull sheet carries a `wreck` frame. A destroyed Shachaf or Kipod leaves the 2D wreck sprite, the shipped `apc_eitan` behaviour, no longer the bare grey cross. Wreck GEOMETRY is the roster-wide mesh-vehicle debt (CLAUDE.md), not this unit's |
