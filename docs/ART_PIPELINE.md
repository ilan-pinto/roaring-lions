...
What DID change, in what a hex means:

- **Authored colour still comes from here, as the INPUT to lighting — not as the output.** A palette hex is decoded to linear, lit, ACES-compressed and re-encoded to sRGB before it reaches the screen, and ACES at exposure 1.0 is not the identity: measured, `#14150F` (`shadow.1`, the clear colour) photographs as `#050503`, 15/255 darker than `main` drew it (spec deviation 7). A saturated key is pulled further than a desaturated one, and a lit surface can land above its authored value as easily as below. The old pass-through pipeline made "authored hex equals screen hex" true by refusing to light anything; the lit pipeline gives that up on purpose, and the palette's job is now to say what a surface *is*, not what a pixel *reads*.
- **One consequence worth naming: the overlay tier is tone-mapped and the DOM HUD is not.** HP bars, selection rings, badge numerals and zone outlines are world geometry, so they go through ACES with everything else; `pnpm validate:ui`'s CSS colours are painted by the browser straight from `--rl-*`. An HTML swatch and an in-world overlay authored from the same palette key therefore no longer match exactly. See the spec's Phase 4 note for the two options.
