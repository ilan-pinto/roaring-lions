...
Updated: 2026-09-18 · main: dd7097bb (v0.71.1) · plan: https://claude.ai/artifact/SND5uxua1RtGy82cxR3JC7 · stage: 0 · next milestone: M1 30 Oct · GitHub: milestones EP M1–M4 (3–6), gate issues #164–#169, package issues #170–#191
...
|---|---|---|---|---|---|
| A (packages/app) | — | — | — | idle | write the shell Phase 2 plan once G0 answers (open with the `MissionEvent` audit) |
| B (render · art · data) | — | — | — | idle | before-captures of the mortar impact and a vehicle death (still + 10 s); apply G-E1 (#173) when G0 #9 answers |
| C (packages/sim) | — | — | — | closed until Stage 4 | — |

## 2. Next — ordered; a fresh session starts at the top of its lane

- A: S2a #170 → S2b #171 (one branch, one bless) → S3e #178 app half → S3e scene host (after A1.3 lands) → S3a #180 → S-F #184, A4 #186
...
Answer on the gate issues: G0 #164 · G1 #165 · G2 #166 · G3 #167 · G4 #168 · G5 #169. Answers are copied here with the date, then the issue is closed.

| Gate | # | Question (short) | Default | Due | Answer · date |
|---|---|---|---|---|---|
| G0 | 1 | Licence "all rights reserved" for art and data (D-16) | keep | 20 Sep | — |
| G0 | 2 | `JEEP_HULL` sheet licence unverified (D-26) | retire with the sheets in A3.3 | 20 Sep | — |
| G0 | 3 | CVD team variants only 1.5 ΔE better (D-22) | keep, relabel, no perceptual claim | 20 Sep | — |
| G0 | 4 | Pixi WebGL leak on soft leave (D-25) | one mission per tab; retire at A3.3 | 20 Sep | — |
| G0 | 5 | Eight KDF units have no `blurb` | narrative-designer writes them in Stage 1 | 20 Sep | — |
| G0 | 6 | Garage Buy below the fold at 1080p | keep the fade; revisit in S3a | 20 Sep | — |
| G0 | 7 | `plates:units` one browser per unit (D-24) | accept | 20 Sep | — |
| G0 | 8 | A ★-only player never sees Shachaf/Kipod | confirm | 20 Sep | — |
| G0 | 9 | Nine Conduct floors open after mission one | raise to 70–90, re-pin the ladder | 20 Sep | — |
| G0 | 10 | `breach_team` 850 vs the 1,550 cap | keep 850, record the exception | 20 Sep | — |
| G0 | 11 | Bought-only special forces: count, bands, CONCEPTS | 2 units, 4,000–4,500 and 6,500–8,000; **concepts have no default (blocking for E5)** | 19 Oct | — |
| G0 | 12 | Roster and reserve cap | measure a ★★★ ladder first; cap above it; overflow to a reserve list | 20 Sep | — |
| G0 | 13 | Trim infantry falls to 0.9–1.2 s | trim inside A3.1 | 20 Sep | — |
| G0 | 14 | The blast's look; mortar shares it | accept the proposal; mortar shares at its own power; judge on 10 s of motion | 20 Sep | — |
...
DECISIONS THIS SESSION: none taken. G0's fourteen are open with defaults (§3 above); default day 21 Sep.
NEXT STEP (exact):
  1. Read G0 (#164) for answers; accept "defaults" as an answer; record answers in §3 with the date; unanswered items default on 21 Sep
